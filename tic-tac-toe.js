class TicTacToe {
	...
	updateScore() {
        ...
        scoreElement.innerHTML = `Player L: ${this.scores["L"]} - Player R: ${this.scores["R"]}`;
    }
	init() {
		this.buildGameUI();
	}
	...
	checkWinner() {
		...
		for (let i = 0; i < winConditions.length; i++) {
			...
			if (
				...
				Swal.fire({
					title: "Good Jobs",
					text: `Congrats Player ${this.currentPlayer + 1}
						You are a winner!`,
					showDenyButton: true,
					...
					if (result.isDenied) {
						this.gameReset();
						Swal.fire("lets try your game!", "", "info");
					}
				});
			}
		}
